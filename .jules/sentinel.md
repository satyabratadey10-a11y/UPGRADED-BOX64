...
## 2024-05-24 - Fix Unbounded `sprintf` Calls in Path Buffers
**Vulnerability:** Found multiple instances of unbounded `sprintf` used to format strings into fixed-size buffers, which risks buffer overflows.
**Learning:** Path strings (e.g., from `/proc/self/fd`) and dynamic filenames need explicitly bounded formatting functions (`snprintf`).
**Prevention:** Strictly enforce the use of `snprintf` with dynamic lengths or `sizeof(buffer)` across the codebase rather than unbounded `sprintf` for buffers handling file paths or system identifiers.
