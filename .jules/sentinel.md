...
## 2026-09-03 - [Buffer Overflow Prevention]
**Vulnerability:** Usage of `sprintf` can lead to buffer overflows if the resulting string is larger than the destination buffer.
**Learning:** Hardcoded bounds are risky because buffer declarations can change in the future.
**Prevention:** Strictly use `snprintf` with `sizeof(buffer)` or dynamically calculated lengths instead of `sprintf`.
