...
const char* getAddrFunctionName(uintptr_t addr)
{
    ...
    static int idx = 0;
    char* ret = rets[idx];
    size_t ret_size = sizeof(rets[idx]);
    idx = (idx + 1) & 7;
    uint64_t sz = 0;
    uintptr_t start = 0;
    ...
    if (symbname && (addr >= start) && (addr < (start + sz))) {
        if (symbname[0] == '\0')
            snprintf(ret, ret_size, "%s + 0x%lx + 0x%lx", ElfName(elf), start - (uintptr_t)GetBaseAddress(elf), addr - start);
        else if (addr == start)
            snprintf(ret, ret_size, "%s/%s", ElfName(elf), symbname);
        else
            snprintf(ret, ret_size, "%s/%s + 0x%lx", ElfName(elf), symbname, addr - start);
    } else {
        if (elf) {
            snprintf(ret, ret_size, "%s + 0x%lx", ElfName(elf), addr - (uintptr_t)GetBaseAddress(elf));
        } else if(IsAddrFileMapped(addr, &symbname, &start)) {
            snprintf(ret, ret_size, "%s+0x%lx", symbname, addr-start);
        } else {
            snprintf(ret, ret_size, "???");
        }
    }
    return ret;
}
